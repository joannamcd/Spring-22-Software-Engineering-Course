...
from controllers.readMongo import readMongoCollection
from controllers.registerUser import registerUser
from controllers.writeMongo import updateMongoCollection
from projClasses.Users import FleetManagers
...
class httpHdlrCmBackEnd(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # endpoint for fleetManager
        if path == '/fleetManagerRegister':
            ...
            fleetManager = FleetManagers(postBody.get("firstName"),
            ...
            isUserExist = isUserRecord(fleetManager.user_name)
            if isUserExist == "userNotExist":
                response = registerUser(fleetManager)
        ...
        elif path == '/vehicleDBUpdate':
            # This is where you'll implement your actual service logic
            # Process any POST data you are expecting to receive
            routeUpdate = postBody
            updateMongoCollection(routeUpdate)
            response = "db updated"
        ...
        elif path == '/vehicleDBRead':
            # This is where you'll implement your actual service logic
            # Process any POST data you are expecting to receive
            routeInfo = readMongoCollection()
            response = routeInfo
            # Collect any data you want to send back into a dictionary and send it back in the responseBody
